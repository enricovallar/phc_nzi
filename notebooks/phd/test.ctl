(set! num-bands 14)
(set! resolution (vector3 25.0 25.0 10.0))
(set! k-points (list (vector3 0.0 0.0 0.0)))
(set! k-points (interpolate 4 k-points))
(define-param h 0.5)
(define-param r1 0.1)
(define-param r2 0.2)
(define-param x_dist 0.25)
...
(set! geometry (list 
 (make block (size (vector3 1 1 h))
  ...
  (material (make dielectric-anisotropic (epsilon-diag (vector3 4.536899999999999 4.536899999999999 2.7224999999999997)))))
 ...
 (make cylinder (radius r2)
  (height h)
  (center (vector3 x_dist 0 0))
  (material (make dielectric (epsilon 1))))
 (make cylinder (radius r2)
  (height h)
  (center (vector3 0 x_dist 0))
  (material (make dielectric (epsilon 1))))
 (make cylinder (radius r2)
  (height h)
  (center (vector3 (* -1 x_dist) x_dist 0))
  (material (make dielectric (epsilon 1))))
 (make cylinder (radius r2)
  (height h)
  (center (vector3 (* -1 x_dist) 0 0))
  (material (make dielectric (epsilon 1))))
 (make cylinder (radius r2)
  (height h)
  (center (vector3 0 (* -1 x_dist) 0))
  (material (make dielectric (epsilon 1))))
 (make cylinder (radius r2)
  (height h)
  (center (vector3 x_dist (* -1 x_dist) 0))
  (material (make dielectric (epsilon 1))))
))
...
(run-zeven display-symmetries-c6v)